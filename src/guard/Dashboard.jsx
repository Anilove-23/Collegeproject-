...
export default function GuardDashboard() {
  ...
  useEffect(() => {
    // Start background sync engine; pass reloadFromDb so it can
    // notify React after pulling fresh data from server
    initSyncEngine(reloadFromDb);

    // Read local cache immediately (works offline)
    reloadFromDb();
  ...
  async function handleGateAction(record, e) {
    ...
    try {
      ...
      // 1. Enqueue action to local Dexie queue — instant, no network needed
      ...
      // 2. Update local outpass status or delete from Dexie
      if (targetAction === "enter") {
        // Student returned — permanently delete from local cache so it
        // doesn't reappear on refresh while the server is still stale.
        await deleteOutpassFromCache(outpassId);
      } else {
        await updateLocalOutpassStatus(outpassId, "Out");
      }

      // 3. Update React state instantly
      if (targetAction === "enter") {
      ...
      // 4. Try to flush queue immediately if online
    ...
    <div className="max-w-7xl mx-auto font-sans text-gray-800 space-y-4 p-4 sm:p-6 relative pb-24">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 pb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          {/* Pending sync badge */}
          {pendingCount > 0 && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold bg-orange-50 text-orange-700 border border-orange-200">
              ⏳ {pendingCount} pending sync
            </div>
          )}

          {/* Stats */}
          <div className="bg-blue-50/80 border border-blue-200 rounded-xl px-3 py-1.5 text-center min-w-[80px]">
          ...
          <div className="bg-amber-50/80 border border-amber-200 rounded-xl px-3 py-1.5 text-center min-w-[80px]">
      ...
      <div className="bg-white border border-gray-200/90 rounded-2xl p-3.5 space-y-3 shadow-xs">
        <div className="flex flex-col md:flex-row items-center gap-2.5">
          ...
          >
            ...
            {[...new Set((Array.isArray(data) ? data : []).map((o) => o.hostel).filter(Boolean))].map((h) => (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2.5 border-t border-gray-100 text-xs">
              ...
              {["All", "In", "Out"].map((s) => (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition cursor-pointer ${
                    status === s ? "bg-[#6d0f16] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  {s}
                </button>
          ...
          <div className="flex items-center gap-1">
            <span className="text-[10px] font-bold uppercase text-gray-400 mr-1">Pass Type:</span>
      ...
      {loading ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center text-gray-500 shadow-xs flex flex-col items-center justify-center space-y-2">
          <div className="w-6 h-6 border-3 border-[#6d0f16] border-t-transparent rounded-full animate-spin" />
                ...
                >
                  ...
                  <div className="bg-gray-50/80 p-2 rounded-xl border border-gray-100 text-[11px] space-y-1">
                    <div className="flex justify-between text-gray-600">
                      ...
                      <span className="font-semibold truncate max-w-[140px]">{o.place_of_visit || "-"}</span>
                    </div>
                    <div className="flex justify-between text-gray-600">
                      <span className="text-gray-400 font-bold">Return:</span>
                      <span className="font-semibold truncate">{formatDate(o.arrival_datetime)}</span>
                    </div>
                  </div>
